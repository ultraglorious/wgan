...
def generator(latent_dim: int) -> tf.keras.Model:
    ...
    x = ConvBlock(5, 2, 128, transpose=True, normalize=False, dropout=True, leaky_slope=ls)(x)  # 14, 14, 128
    x = ConvBlock(5, 2, 64, transpose=True, normalize=False, dropout=True, leaky_slope=ls)(x)  # 7, 7, 64
    x = ConvBlock(5, 1, 1, transpose=True, normalize=False, dropout=True, activation="tanh")(x)  # 7, 7, 1
